import { $n as toRefs$1, An as customRef, At as onBeforeMount, Bn as markRaw, Bt as onUpdated, Ft as onMounted, Gn as readonly, In as isReactive, Kn as ref, Ln as isReadonly, M as Fragment, Mn as effectScope, Mt as onBeforeUpdate, Nn as getCurrentScope, Ot as nextTick, Qn as toRef$1, Rn as isRef, U as computed, Ut as provide, Vn as onScopeDispose, Wn as reactive, Yn as shallowRef, _n as watchEffect, gn as watch, jt as onBeforeUnmount, mn as version, nr as unref, nt as defineComponent, pt as h, qn as shallowReactive, r as TransitionGroup, ut as getCurrentInstance, xt as inject, zt as onUnmounted } from "./vue.runtime.esm-bundler-2KvXzv-8.js";

//#region ../../node_modules/.pnpm/vue-demi@0.14.10_vue@3.5.26_typescript@5.9.3_/node_modules/vue-demi/lib/index.mjs
var isVue2 = false;
var isVue3 = true;
...
//#endregion
//#region ../../node_modules/.pnpm/@vueuse+shared@10.11.1_vue@3.5.26_typescript@5.9.3_/node_modules/@vueuse/shared/index.mjs
...
function computedWithControl(source, fn) {
	...
	const set$2 = typeof fn === "function" ? void 0 : fn.set;
...
function extendRef(ref$1, extend, { enumerable = false, unwrap = true } = {}) {
	if (!isVue3 && !version.startsWith("2.7.")) throw new Error("[VueUse] extendRef only works in Vue 2.7 or above.");
...
function refWithControl(initial, options = {}) {
	...
	function set$2(value, triggering = true) {
		var _a, _b;
	...
	const silentSet = (v) => set$2(v, false);
	const peek = () => get$1(false);
	const lay = (v) => set$2(v, false);
...
function set$1(...args) {
	...
	if (args.length === 3) if (isVue2) set(...args);
	else {
...
function useCounter(initialValue = 0, options = {}) {
	...
	const set$2 = (val) => count.value = Math.max(min, Math.min(max, val));
...
//#endregion
//#region ../../node_modules/.pnpm/@vueuse+core@10.11.1_vue@3.5.26_typescript@5.9.3_/node_modules/@vueuse/core/index.mjs
...
function createReusableTemplate(options = {}) {
	if (!isVue3 && !version.startsWith("2.7.")) throw new Error("[VueUse] createReusableTemplate only works in Vue 2.7 or above.");
...
function createTemplatePromise(options = {}) {
	if (!isVue3) throw new Error("[VueUse] createTemplatePromise only works in Vue 3 or above.");
	let index = 0;
	const instances = ref([]);
...
function useEventListener(...args) {
	...
	if (typeof args[0] === "string" || Array.isArray(args[0])) {
		[events$1, listeners, options] = args;
		target = defaultWindow;
	} else [target, events$1, listeners, options] = args;
	if (!target) return noop;
	if (!Array.isArray(events$1)) events$1 = [events$1];
	...
	const stopWatch = watch(() => [unrefElement(target), toValue(options)], ([el, options2]) => {
		...
		cleanups.push(...events$1.flatMap((event) => {
...
function useAnimate(target, keyframes, options) {
	...
	watch(() => keyframes, (value) => {
		!animate.value && update();
		if (!unrefElement(target) && animate.value) animate.value.effect = new KeyframeEffect(unrefElement(target), toValue(value), animateOptions);
	}, { deep: true });
	tryOnMounted(() => {
...
function useBreakpoints(breakpoints, options = {}) {
	...
	const { window: window$1 = defaultWindow, strategy = "min-width" } = options;
	...
	return Object.assign(shortcutMethods, {
		...
		isGreater(k) {
			return match(`(min-width: ${getValue$1(k, .1)})`);
		},
		isGreaterOrEqual(k) {
			return match(`(min-width: ${getValue$1(k)})`);
		},
		isSmaller(k) {
			return match(`(max-width: ${getValue$1(k, -.1)})`);
		},
		isSmallerOrEqual(k) {
			return match(`(max-width: ${getValue$1(k)})`);
		},
		isInBetween(a, b) {
			return match(`(min-width: ${getValue$1(a)}) and (max-width: ${getValue$1(b, -.1)})`);
		},
		current,
		active() {
			const bps = current();
...
function useBrowserLocation(options = {}) {
	...
	for (const [key, ref2] of objectEntries(refs)) watch(ref2, (value) => {
		if (!(window$1 == null ? void 0 : window$1.location) || window$1.location[key] === value) return;
		window$1.location[key] = value;
	});
	const buildState = (trigger) => {
		var _a;
		const { state: state2, length } = (window$1 == null ? void 0 : window$1.history) || {};
		const { origin } = (window$1 == null ? void 0 : window$1.location) || {};
		for (const key of WRITABLE_PROPERTIES) refs[key].value = (_a = window$1 == null ? void 0 : window$1.location) == null ? void 0 : _a[key];
		return reactive({
			trigger,
			state: state2,
			length,
	...
	if (window$1) {
		useEventListener(window$1, "popstate", () => state.value = buildState("popstate"), { passive: true });
		useEventListener(window$1, "hashchange", () => state.value = buildState("hashchange"), { passive: true });
	}
	return state;
}
...
function useCycleList(list, options) {
	...
	function shift(delta = 1) {
		return set$2(index.value + delta);
	}
	function next(n = 1) {
		return shift(n);
	}
	...
	watch(listRef, () => set$2(index.value));
	return {
		state,
		index,
		next,
...
function useDark(options = {}) {
	const { valueDark = "dark", valueLight = "", window: window$1 = defaultWindow } = options;
	...
	const system = computed(() => {
		...
		else return usePreferredDark({ window: window$1 }).value ? "dark" : "light";
	});
	return computed({
		get() {
...
function useDeviceMotion(options = {}) {
	const { window: window$1 = defaultWindow, eventFilter = bypassFilter } = options;
...
function useDevicePixelRatio(options = {}) {
	...
	if (window$1) {
		let observe2 = function() {
			...
			cleanup2();
			media = window$1.matchMedia(`(resolution: ${pixelRatio.value}dppx)`);
...
function useDraggable(target, options = {}) {
	var _a, _b;
	const { pointerTypes, preventDefault: preventDefault$1, stopPropagation, exact, onMove, onEnd, onStart, initialValue, axis = "both", draggingElement = defaultWindow, containerElement, handle: draggingHandle = target } = options;
...
function useElementByPoint(options) {
	...
	const cb = () => {
		var _a, _b;
		element.value = toValue(multiple) ? (_a = document$1 == null ? void 0 : document$1.elementsFromPoint(toValue(x), toValue(y))) != null ? _a : [] : (_b = document$1 == null ? void 0 : document$1.elementFromPoint(toValue(x), toValue(y))) != null ? _b : null;
	};
	return {
		isSupported,
		element,
...
function useIntersectionObserver(target, callback, options = {}) {
	const { root, rootMargin = "0px", threshold = .1, window: window$1 = defaultWindow, immediate = true } = options;
...
function useElementVisibility(element, options = {}) {
	const { window: window$1 = defaultWindow, scrollTarget, threshold = 0 } = options;
...
function useFocusWithin(target, options = {}) {
	...
	const targetElement = computed(() => unrefElement(target));
	return { focused: computed(() => targetElement.value && activeElement.value ? targetElement.value.contains(activeElement.value) : false) };
}
...
function useFullscreen(target, options = {}) {
	...
	const targetRef = computed(() => {
		var _a;
		return (_a = unrefElement(target)) != null ? _a : document$1 == null ? void 0 : document$1.querySelector("html");
	});
	...
	const isCurrentElementFullScreen = () => {
		if (fullscreenElementMethod) return (document$1 == null ? void 0 : document$1[fullscreenElementMethod]) === targetRef.value;
		return false;
	};
	...
	async function exit() {
		...
		if (exitMethod.value) if ((document$1 == null ? void 0 : document$1[exitMethod.value]) != null) await document$1[exitMethod.value]();
		else {
			const target2 = targetRef.value;
	...
	useEventListener(document$1, eventHandlers, handlerCallback, false);
...
function useIdle(timeout = oneMinute, options = {}) {
	...
	if (window$1) {
		...
		for (const event of events$1) useEventListener(window$1, event, onEvent, { passive: true });
...
function useMagicKeys(options = {}) {
	...
	const proxy = new Proxy(refs, { get(target2, prop, rec) {
		...
		if (!(prop in refs)) if (/[+_-]/.test(prop)) {
			...
			refs[prop] = computed(() => keys$1.every((key) => toValue(proxy[key])));
		} else refs[prop] = ref(false);
...
function useMouse(options = {}) {
	...
	const scrollHandler = () => {
		...
		if (_prevMouseEvent instanceof MouseEvent && pos) {
			x.value = pos[0] + window$1.scrollX;
			y.value = pos[1] + window$1.scrollY;
		}
	};
	const reset = () => {
	...
	if (target) {
		...
		if (scroll && type === "page") useEventListener(window$1, "scroll", scrollHandlerWrapper, { passive: true });
	}
	return {
		x,
		y,
...
function useMouseInElement(target, options = {}) {
	...
	const targetRef = ref(target != null ? target : window$1 == null ? void 0 : window$1.document.body);
...
function useMousePressed(options = {}) {
	...
	useEventListener(window$1, "mouseleave", onReleased, {
		passive: true,
		capture
	});
	useEventListener(window$1, "mouseup", onReleased, {
		passive: true,
		capture
	});
	if (drag) {
		...
		useEventListener(window$1, "drop", onReleased, {
			passive: true,
			capture
		});
		useEventListener(window$1, "dragend", onReleased, {
			passive: true,
			capture
		});
	}
	if (touch) {
		...
		useEventListener(window$1, "touchend", onReleased, {
			passive: true,
			capture
		});
		useEventListener(window$1, "touchcancel", onReleased, {
			passive: true,
			capture
		});
	}
...
function useNavigatorLanguage(options = {}) {
	const { window: window$1 = defaultWindow } = options;
	const navigator = window$1 == null ? void 0 : window$1.navigator;
...
function usePageLeave(options = {}) {
	...
	if (window$1) {
		useEventListener(window$1, "mouseout", handler, { passive: true });
		useEventListener(window$1.document, "mouseleave", handler, { passive: true });
		useEventListener(window$1.document, "mouseenter", handler, { passive: true });
	}
	return isLeft;
}
...
function usePointerLock(target, options = {}) {
	...
	if (isSupported.value) {
		useEventListener(document$1, "pointerlockchange", () => {
			var _a;
			const currentElement = (_a = document$1.pointerLockElement) != null ? _a : element.value;
...
function useScriptTag(src, onLoaded = noop, options = {}) {
	const { immediate = true, manual = false, type = "text/javascript", async = true, crossOrigin, referrerPolicy, noModule, defer, document: document$1 = defaultDocument, attrs = {} } = options;
...
function useStorageAsync(key, initialValue, storage, options = {}) {
	...
	read();
	if (window$1 && listenToStorageChanges) useEventListener(window$1, "storage", (e) => Promise.resolve().then(() => read(e)));
...
function useSwipe(target, options = {}) {
	const { threshold = 50, onSwipe, onSwipeEnd, onSwipeStart, passive = true, window: window$1 = defaultWindow } = options;
	...
	let listenerOptions;
	const isPassiveEventSupported = checkPassiveEventSupport(window$1 == null ? void 0 : window$1.document);
...
function checkPassiveEventSupport(document$1) {
	if (!document$1) return false;
	let supportsPassive = false;
	document$1.addEventListener("x", noop, { get passive() {
		supportsPassive = true;
		return false;
	} });
	document$1.removeEventListener("x", noop);
...
function useTextDirection(options = {}) {
	...
	function getValue$1() {
		var _a, _b;
		return (_b = (_a = document$1 == null ? void 0 : document$1.querySelector(selector)) == null ? void 0 : _a.getAttribute("dir")) != null ? _b : initialValue;
	}
	...
	return computed({
		...
		set(v) {
			...
			dir.value = v;
			if (!document$1) return;
			if (dir.value) (_a = document$1.querySelector(selector)) == null || _a.setAttribute("dir", dir.value);
			else (_b = document$1.querySelector(selector)) == null || _b.removeAttribute("dir");
		}
	});
}
...
function useTextSelection(options = {}) {
	...
	if (window$1) useEventListener(window$1.document, "selectionchange", onSelectionChange);
	return {
		text,
		rects,
		ranges,
...
function useTextareaAutosize(options) {
	...
	function triggerResize() {
		...
		if (options == null ? void 0 : options.styleTarget) toValue(options.styleTarget).style[styleProp] = `${textareaScrollHeight.value}px`;
...
function useTitle(newTitle = null, options = {}) {
	...
	const originalTitle = (_a = document$1 == null ? void 0 : document$1.title) != null ? _a : "";
	const title = toRef((_b = newTitle != null ? newTitle : document$1 == null ? void 0 : document$1.title) != null ? _b : null);
	...
	watch(title, (t, o) => {
		if (t !== o && document$1) document$1.title = format(typeof t === "string" ? t : "");
	}, { immediate: true });
	if (options.observe && !options.titleTemplate && document$1 && !isReadonly$1) useMutationObserver((_c = document$1.head) == null ? void 0 : _c.querySelector("title"), () => {
...
function useUrlSearchParams(mode = "history", options = {}) {
	const { initialValue = {}, removeNullishValues = true, removeFalsyValues = false, write: enableWrite = true, window: window$1 = defaultWindow } = options;
	...
	function write(params, shouldUpdate) {
		...
		window$1.history.replaceState(window$1.history.state, window$1.document.title, window$1.location.pathname + constructQuery(params));
		resume();
	}
	function onChanged() {
	...
	useEventListener(window$1, "popstate", onChanged, false);
	if (mode !== "history") useEventListener(window$1, "hashchange", onChanged, false);
	const initial = read();
...
function useWakeLock(options = {}) {
	const { navigator = defaultNavigator, document: document$1 = defaultDocument } = options;
	let wakeLock;
	const isSupported = useSupported(() => navigator && "wakeLock" in navigator);
	const isActive = ref(false);
	async function onVisibilityChange() {
		if (!isSupported.value || !wakeLock) return;
		if (document$1 && document$1.visibilityState === "visible") wakeLock = await navigator.wakeLock.request("screen");
		isActive.value = !wakeLock.released;
	}
	if (document$1) useEventListener(document$1, "visibilitychange", onVisibilityChange, { passive: true });
...
function useWindowScroll(options = {}) {
	const { window: window$1 = defaultWindow, behavior = "auto" } = options;
	if (!window$1) return {
		x: ref(0),
		y: ref(0)
	};
	const internalX = ref(window$1.scrollX);
	const internalY = ref(window$1.scrollY);
	const x = computed({
		get() {
			return internalX.value;
		},
	...
	useEventListener(window$1, "scroll", () => {
		internalX.value = window$1.scrollX;
		internalY.value = window$1.scrollY;
	}, {
		capture: false,
		passive: true
	});
...
function useWindowSize(options = {}) {
	const { window: window$1 = defaultWindow, initialWidth = Number.POSITIVE_INFINITY, initialHeight = Number.POSITIVE_INFINITY, listenOrientation = true, includeScrollbar = true } = options;
	const width = ref(initialWidth);
	const height = ref(initialHeight);
	const update = () => {
		if (window$1) if (includeScrollbar) {
...
//#endregion
export { useCloned as $, watchOnce as $i, useWebNotification as $n, reactiveOmit as $r, useNow as $t, onClickOutside as A, useArrayJoin as Ai, useStepper as An, injectLocal as Ar, useFps as At, useActiveElement as B, useLastChanged as Bi, useThrottledRefHistory as Bn, normalizeDate as Br, useMagicKeys as Bt, defaultLocation as C, useArrayDifference as Ci, useScroll as Cn, formatDate as Cr, useEyeDropper as Ct, formatTimeAgo as D, useArrayFindIndex as Di, useSorted as Dn, hyphenate as Dr, useFileSystemAccess as Dt, executeTransition as E, useArrayFind as Ei, useShare as En, hasOwn as Er, useFileDialog as Et, onLongPress as F, useCounter as Fi, useSwipe as Fn, isIOS as Fr, useImage as Ft, useBattery as G, useToString as Gi, useTransition as Gn, objectPick as Gr, useMemory as Gt, useAsyncQueue as H, useTimeout as Hi, useTimeoutPoll as Hn, now as Hr, useMediaControls as Ht, onStartTyping as I, useDateFormat as Ii, useTemplateRefsList as In, isObject as Ir, useInfiniteScroll as It, useBroadcastChannel as J, watchAtMost as Ji, useVModel as Jn, provideLocal as Jr, useMouseInElement as Jt, useBluetooth as K, useToggle as Ki, useUrlSearchParams as Kn, pausableFilter as Kr, useMounted as Kt, setSSRHandler as L, useDebounceFn as Li, useTextDirection as Ln, isWorker as Lr, useIntersectionObserver as Lt, onKeyPressed as M, useArrayReduce as Mi, useStorageAsync as Mn, isClient as Mr, useGamepad as Mt, onKeyStroke as N, useArraySome as Ni, useStyleTag as Nn, isDef as Nr, useGeolocation as Nt, getSSRHandler as O, useArrayFindLast as Oi, useSpeechRecognition as On, identity as Or, useFocus as Ot, onKeyUp as P, useArrayUnique as Pi, useSupported as Pn, isDefined as Pr, useIdle as Pt, useClipboardItems as Q, watchImmediate as Qi, useWakeLock as Qn, reactiveComputed as Qr, useNetwork as Qt, templateRef as R, useInterval as Ri, useTextSelection as Rn, makeDestructurable as Rr, useKeyModifier as Rt, defaultDocument as S, until as Si, useScriptTag as Sn, extendRef as Sr, useEventSource as St, defaultWindow as T, useArrayFilter as Ti, useSessionStorage as Tn, getLifeCycleTarget as Tr, useFetch as Tt, useAsyncState as U, useTimeoutFn as Ui, useTimestamp as Un, objectEntries as Ur, useMediaQuery as Ut, useAnimate as V, useThrottleFn as Vi, useTimeAgo as Vn, notNullish as Vr, useManualRefHistory as Vt, useBase64 as W, useToNumber as Wi, useTitle as Wn, objectOmit as Wr, useMemoize as Wt, useCached as X, watchDeep as Xi, useVibrate as Xn, reactify as Xr, useMutationObserver as Xt, useBrowserLocation as Y, watchDebounced as Yi, useVModels as Yn, rand as Yr, useMousePressed as Yt, useClipboard as Z, watchIgnorable as Zi, useVirtualList as Zn, reactifyObject as Zr, useNavigatorLanguage as Zt, createFetch as _, tryOnBeforeMount as _i, useRafFn as _n, createInjectionState as _r, useElementHover as _t, breakpointsBootstrapV5 as a, refWithControl as ai, useParentElement as an, useWindowSize as ar, useDark as at, createUnrefFn as b, tryOnScopeDispose as bi, useScreenOrientation as bn, debounceFilter as br, useEventBus as bt, breakpointsQuasar as c, set$1 as ci, usePointer as cn, camelize as cr, useDeviceOrientation as ct, breakpointsVuetify as d, throttleFilter as di, usePreferredColorScheme as dn, computedWithControl as dr, useDisplayMedia as dt, watchPausable as ea, reactivePick as ei, useObjectUrl as en, useWebSocket as er, useColorMode as et, breakpointsVuetifyV2 as f, timestamp as fi, usePreferredContrast as fn, containsProp as fr, useDocumentVisibility as ft, computedInject as g, toValue as gi, usePrevious as gn, createGlobalState as gr, useElementByPoint as gt, computedAsync as h, toRefs as hi, usePreferredReducedMotion as hn, createFilterWrapper as hr, useElementBounding as ht, breakpointsAntDesign as i, whenever as ia, refThrottled as ii, useParallax as in, useWindowScroll as ir, useCycleList as it, onKeyDown as j, useArrayMap as ji, useStorage as jn, invoke as jr, useFullscreen as jt, mapGamepadToXbox360Controller as k, useArrayIncludes as ki, useSpeechSynthesis as kn, increaseWithUnit as kr, useFocusWithin as kt, breakpointsSematic as l, syncRef as li, usePointerLock as ln, clamp as lr, useDevicePixelRatio as lt, cloneFnJSON as m, toRef as mi, usePreferredLanguages as mn, createEventHook as mr, useDropZone as mt, StorageSerializers as n, watchTriggerable as na, refDebounced as ni, useOnline as nn, useWebWorkerFn as nr, useCssVar as nt, breakpointsMasterCss as o, resolveRef as oi, usePerformanceObserver as on, assert as or, useDebouncedRefHistory as ot, breakpointsVuetifyV3 as p, toReactive as pi, usePreferredDark as pn, controlledRef as pr, useDraggable as pt, useBreakpoints as q, watchArray as qi, useUserMedia as qn, promiseTimeout as qr, useMouse as qt, TransitionPresets as r, watchWithFilter as ra, refDefault as ri, usePageLeave as rn, useWindowFocus as rr, useCurrentElement as rt, breakpointsPrimeFlex as s, resolveUnref as si, usePermission as sn, bypassFilter as sr, useDeviceMotion as st, DefaultMagicKeysAliasMap as t, watchThrottled as ta, refAutoReset as ti, useOffsetPagination as tn, useWebWorker as tr, useConfirmDialog as tt, breakpointsTailwind as u, syncRefs as ui, usePointerSwipe as un, computedEager as ur, useDevicesList as ut, createReusableTemplate as v, tryOnBeforeUnmount as vi, useRefHistory as vn, createSharedComposable as vr, useElementSize as vt, defaultNavigator as w, useArrayEvery as wi, useScrollLock as wn, get as wr, useFavicon as wt, customStorageEventName as x, tryOnUnmounted as xi, useScreenSafeArea as xn, directiveHooks as xr, useEventListener as xt, createTemplatePromise as y, tryOnMounted as yi, useResizeObserver as yn, createSingletonPromise as yr, useElementVisibility as yt, unrefElement as z, useIntervalFn as zi, useTextareaAutosize as zn, noop as zr, useLocalStorage as zt };
//# sourceMappingURL=core-jmwJ8XwP.js.map